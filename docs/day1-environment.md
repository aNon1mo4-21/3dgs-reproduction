...
本机尚不满足官方训练/CUDA 渲染环境；未安装训练依赖，未下载场景，未训练或渲染。这是租用 GPU 前的检查结果；后续已完成远端基础环境验证，见 [GPU 实例检查](day1-gpu-environment.md)。
...
third_party/gaussian-splatting 是官方仓库的 Git 子模块。父仓库记录它的精确提交，避免上游更新后实验代码悄悄改变；我将环境记录和实验说明放在父仓库。
...
官方 environment.yml 固定 Python 3.7.13、PyTorch 1.12.1 和 cudatoolkit 11.6；README 同时提及 CUDA SDK 11.8 与 11.6 的已知问题。我未在本地 Python 3.14 全局环境中安装训练依赖。远端已选择 Python 3.10.8、PyTorch 2.1.2+cu118 和 CUDA Toolkit 11.8；官方扩展兼容性仍待编译验证。
...
我将在完成首个场景后，结合输入文件、训练过程和渲染结果整理对完整流程的理解。
