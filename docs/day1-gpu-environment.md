...
我通过 SSH 连接租赁实例，并执行以下检查。记录基于实例实际输出，不采用租赁页面的标称配置。
...
官方源码从本地完整仓库打包传输（含 Git 元数据及已初始化子模块），无需重新选择上游版本。传输归档位于仓库之外。远端项目路径为 `/root/autodl-tmp/3dgs-reproduction`。主仓库与已初始化依赖的提交均已核验一致。
